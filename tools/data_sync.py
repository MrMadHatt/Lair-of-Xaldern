...
def setup_database():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS room_enemies (
                    room_id INTEGER,
                    enemy_id INTEGER,
                    current_hp INTEGER,
...
def sync_data():
    ...
    tables = ["room_items", "enemy_loot", "room_enemies", "weapons", "enemies", "rooms", "items"]
    for table in tables:
    ...
    for root, dirs, filenames in os.walk(DATA_FOLDER):
        if 'draft' in root.lower(): continue
        for f in filenames:
    ...
    print(f"📊 Files Discovered: {len(files)} | Successfully Parsed: {len(parsed_data)}")
    ...
    for metadata, body, filename in parsed_data:
        ...
        raw_id = metadata.get('id') or metadata.get('item_id') or metadata.get('enemy_id') or metadata.get('weapon_id')
        ...
        if etype in ['item', 'weapon', 'consumable']:
            ...
            cursor.execute('INSERT OR REPLACE INTO items VALUES (?,?,?)', (entry_id, name, status))
            if etype == 'weapon':
                cursor.execute('INSERT OR REPLACE INTO weapons VALUES (?,?,?,?,?,?,?)',
                             (entry_id, name, metadata.get('damage', 0), metadata.get('weight', 0.0),
                              metadata.get('price', 0), metadata.get('health_modifier', 0), status))
            print(f"📦 {etype.capitalize()} {entry_id} synced.")

        elif etype == 'enemy':
            name = metadata.get('name', 'Unknown Enemy')
            hp = metadata.get('hp', 10)
            cursor.execute('INSERT OR REPLACE INTO enemies VALUES (?,?,?,?,?,?,?,?)',
                         (entry_id, name, hp, hp, metadata.get('attack', 2), 
                          metadata.get('defense', 0), body, status))
            ...
            if isinstance(loot_list, list):
                for loot in loot_list:
                    ...
                    if l_id:
                        cursor.execute("INSERT INTO enemy_loot VALUES (?,?,?)", 
    ...
    for metadata, body, filename in parsed_data:
        ...
        if 'room' in filename.lower() or etype in ['room', 'location']:
            if status != 'finished':
                print(f"⚠️  SKIPPING {filename}: Status is '{status}' (Expected 'finished')")
            if not raw_id:
        ...
        n, s, e, w = [clean_id(metadata.get(d)) for d in ['north', 'south', 'east', 'west']]
        ...
        for item_raw in (metadata.get('items') or metadata.get('contains_items') or []):
            ...
            if i_id: cursor.execute("INSERT INTO room_items VALUES (?, ?)", (entry_id, i_id))

        # Room Enemies
        for enemy_raw in (metadata.get('enemies') or metadata.get('contains_enemies') or []):
            e_id = clean_id(enemy_raw)
            if e_id:
                cursor.execute("SELECT health FROM enemies WHERE id = ?", (e_id,))
                res = cursor.fetchone()
                hp = res[0] if res else 10
                cursor.execute("INSERT INTO room_enemies VALUES (?, ?, ?, 1)", (entry_id, e_id, hp))
